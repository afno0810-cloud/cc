...
export function createCoast(density = 1) {
    ...
    const lineMat = (route) =>
        new THREE.ShaderMaterial({
            ...
            vertexShader: /* glsl */ `
                ...
                void main(){ vU = aU; vec4 mv = modelViewMatrix * vec4(position,1.0); gl_PointSize = uPx * ${route ? "3.4" : "2.0"} * (560.0 / -mv.z); gl_Position = projectionMatrix * mv; }
            `,
            fragmentShader: /* glsl */ `
    ...
    // draw after the water (which is transparent and writes depth), or the water covers the map
